...
import Login from "../Useres/Login";
...
import { editCompany } from "../../store/user/actions";
...
export const CreateCompany: React.FC = () => {
  ...
  const apiActions = {
    createCompany: (company: Company): Promise<Company> =>
      requests.post(
        `Company/CreateCompany?username=${verifiedUser?.username}`,
        company
      ),
    ...
    <>
      ...
      {verifiedUser && (
        <>
          ...
          <div
            className="createCompanyStoreContainer"
            style={
              storesAvaliable && !companyInEdit
                ? { opacity: 1 }
                : { opacity: 0.7 }
            }
          >
