...
import { LoginDto, User, VerifiedUser, VerifiedUserClass } from '../../models/User'
import { ThunkAction } from 'redux-thunk'
import { AppState } from '../rootReducer'
import { Action } from 'redux'
import { toggleNotification, turnOffLoading, turnOnLoading } from '../shared/actions'
import { Company } from '../../models/Company'

const apiActions = {
	login: (userLogin: LoginDto): Promise<VerifiedUser> => requests.post(`Security/Login`, userLogin),
	CheckUserToken: (): Promise<boolean> => requests.get(`Security/CheckUserToken`),
	register: (user: User): Promise<void> => requests.post(`User/CreateUser`, user),
	...
	DeleteCompany: (companyId: number): Promise<boolean> => requests.post(`Company/DeleteCompany`, companyId),
	AddCopmanyToUser: (userId: number, companyId: number) => requests.post(`User/AddCompanyToUser?userId=${userId}&companyId=${companyId}`, {}),
	...
	deleteCompany: (companyId: number): Promise<number> => requests.post(`Company/DeleteCompany?companyId=${companyId}`, {}),
}
...
export const getUserCompanies =
	...
	async (dispatch) => {
		try {
			...
			dispatch(GetUserCompaniesSuccess(await apiActions.GetUserCompanies(username)))
...
export const createCompany =
	(company: Company, username: string): ThunkAction<void, AppState, unknown, Action<string>> =>
	async (dispatch) => {
		try {
			dispatch(turnOnLoading())
			dispatch(CreateCompany(await apiActions.CreateCompany(company, username)))
			dispatch(turnOffLoading())
		} catch (error) {
			console.error(error)
			dispatch(turnOffLoading())
...
export const SaveVerifiedUser =
	(verifiedUser: VerifiedUser): ThunkAction<void, AppState, unknown, Action<string>> =>
	async (dispatch) => {
		try {
		...
		function SaveVerifiedUserSuccess(verifiedUser: VerifiedUser): IUserActionType {
			if (verifiedUser.token) {
				document.cookie = `jwt=` + verifiedUser.token + `;path=/`
				verifiedUser.username
					? (document.cookie = `username=` + verifiedUser.username + `; path=/`)
					: dispatch(toggleNotification(true, 'error', 'Token avalible but there is no username'))
			}
			return {
...
export const logOut = (): ThunkAction<void, AppState, unknown, Action<string>> => async (dispatch) => {
	try {
		...
		dispatch(toggleNotification(true, 'error', 'Something went wrong on logOut'))
		console.log(error)
	}
...
export const setTokenIfExists = (): ThunkAction<void, AppState, unknown, Action<string>> => async (dispatch) => {
	try {
		...
		dispatch(toggleNotification(true, 'error', 'Something went wrong trying to set existing token'))
		console.log(error)
	}
}

export function dispatchUserInfo(token: string | undefined, username: string | undefined, role: string | undefined): IUserActionType {
	return {
		type: actionTypes.USER_LOGIN,
